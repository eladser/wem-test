...
if (Test-Path $infrastructureDiPath) {
    Write-Host "ℹ️ Infrastructure DI registration file found" -ForegroundColor Blue
    ...
    if ($missingRepositories.Count -eq 0) {
        Write-Host "✅ All repository services are registered" -ForegroundColor Green
    }
    else {
        Write-Host "⚠️ Some repository services may need registration" -ForegroundColor Yellow
    }
}

if (Test-Path $applicationDiPath) {
    Write-Host "✅ Application DI registration file exists" -ForegroundColor Green
}
else {
    Write-Host "⚠️ Application DI file not found - services may need manual registration" -ForegroundColor Yellow
}
        ...
        if (Test-Command "sqlite3") {
            ...
            foreach ($table in $tables) {
                try {
                    ...
                    Write-Host "⚠️ Could not check table '$table'" -ForegroundColor Yellow
                }
            }
        }
        else {
...
if (Test-Path $BackupPath) {
    Write-Host "`n💾 Database Backup Location: $BackupPath" -ForegroundColor Blue
}
